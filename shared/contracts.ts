...
/** Human label for UI, e.g. "Mainnet" / "Testnet". */
export function networkDisplayLabel(network: string = STELLAR_NETWORK): string {
  if (network === "mainnet" || network === "public" || network === "pubnet") return "Mainnet";
  if (network === "futurenet") return "Futurenet";
  if (network === "standalone") return "Standalone";
  return "Testnet";
}

export const NETWORK_LABEL = networkDisplayLabel(STELLAR_NETWORK);
